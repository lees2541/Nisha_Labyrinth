...
#            해당 플레이어의 생명석 개수
#declare tag/function #nl_system:calc_lifestone 형식: {operator:add|remove, operand: (int)}

execute store result score #temp NL_lifestone_count run scoreboard players get @s NL_lifestone_count
execute if score @s NL_lifestone_count matches ..-1 run return -1
...
execute if score #temp NL_lifestone_count matches ..-1 run return 0
