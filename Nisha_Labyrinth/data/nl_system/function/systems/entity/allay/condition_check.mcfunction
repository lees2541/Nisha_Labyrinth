...
execute if data entity @s Passengers[{id:"minecraft:marker"}].data.id unless entity @a[tag=NL,nbt={SelectedItem:{tag:{lib:1b,prison_key:1b}}},distance=..4] run return run scoreboard players set @s NL_mode 2
#execute if data entity @s Passengers[{id:"minecraft:marker"}].data.id run scoreboard players set @s NL_mode 2


$execute if entity @a[tag=NL,nbt={SelectedItem:{tag:{lib:1b,prison_key:1b}}},distance=..4] run data modify entity @e[type=marker,tag=NL,tag=allay,tag=$(tag),limit=1,sort=nearest] data.id set from entity @a[tag=NL,limit=1,sort=nearest,nbt={SelectedItem:{tag:{lib:1b,prison_key:1b}}},distance=..4] Attributes[{Name:"minecraft:generic.luck"}].Base
