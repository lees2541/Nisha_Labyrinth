...
$execute unless data entity @s Passengers[{id:"minecraft:marker"}].data.id if entity @a[tag=NL,predicate=nl_system:status/action/sprinting,tag=$(tag)] run data modify entity @e[type=marker,tag=NL,tag=allay,tag=$(tag),limit=1,sort=nearest] data.id set from entity @a[tag=NL,tag=$(tag),predicate=nl_system:status/action/sprinting,limit=1,sort=nearest] attributes[{id:"minecraft:generic.luck"}].base
