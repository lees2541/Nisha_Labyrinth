...
execute as @e[type=interaction,tag=NL,tag=occupied,tag=swamp,tag=berry] on passengers if entity @s[type=item_display,tag=NL,tag=swamp] run data modify entity @s item merge value {id:"minecraft:sweet_berries",count:1b}
