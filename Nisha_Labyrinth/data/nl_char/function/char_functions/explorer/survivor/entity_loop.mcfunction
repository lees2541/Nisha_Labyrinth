...
execute as @n[type=interaction,tag=NL,tag=occupied,tag=campfire] at @s unless block ~ ~ ~ campfire run tag @s remove occupied
execute as @n[type=interaction,tag=NL,tag=!occupied, tag=campfire] as @a[tag=explorer,tag=survivor] run function nl_char:char_functions/explorer/survivor/status/remove/remove_campfire with entity @s attributes[{id:"minecraft:luck"}]
