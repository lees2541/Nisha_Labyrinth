...
execute unless data entity @s Passengers[{id:"minecraft:marker"}].data.route_point[0] run return 1

data modify storage nl:buffer position set value [I;]
function nl_lobby:modules/entity_route/get_route_point_pos with entity @s Passengers[{id:"minecraft:marker"}].data.route_point[0]
$data modify entity @e[type=marker,tag=NL,limit=1,sort=nearest,tag=$(tag)] data.route append from storage nl:buffer position
$data remove entity @e[type=marker,tag=NL,limit=1,sort=nearest,tag=$(tag)] data.route_point[0]
